...
Thanks to Max for the help with the script
...
Documentation for each package we use: <br>
...
To start with we download and open the packages we'll be using for this project.
```{r, message=FALSE}
library(tidyverse)
library(tidytext)
library(ggwordcloud)
...
As we can see Tobak is quite frequent in the newspaper which could indicate it was advertised a lot
